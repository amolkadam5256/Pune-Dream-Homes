        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-yellow-400 text-black px-2 py-0.5 mt-2 font-bold text-xs">
                MB PRIME
              </span>
              <h1 className="text-2xl xl:text-4xl font-black">
                Pune <span className="text-yellow-400">Dream</span> Homes Prime
              </h1>
            </div>
            <p className="text-lg sm:text-xl mb-6 font-medium text-blue-50">
          ...
          >
            ...
            <p className="text-2xl font-black text-(--color-primary) mb-1">
              +91-7303439363
            </p>
            ...
            <a
              href="#how-it-works"
              className="w-full bg-(--color-primary) text-white py-3 rounded-md font-bold hover:bg-(--color-primary-dark-1) transition-colors flex items-center justify-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => (
              <motion.div
                key={plan.name}
                whileHover={{ y: -5 }}
                className={`relative bg-white rounded-md shadow-lg overflow-hidden border-2 ${
                ...
                <div className="p-6 space-y-4">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-500">
                      No. of Owners to contact
                    </span>
                    <span className="font-bold">{plan.features.contacts}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-500">
                      Unlock Owner Properties
                    </span>
                    {plan.features.unlock ? (
                      <Check className="text-green-500" size={18} />
                    ) : (
                      <Zap className="text-gray-300" size={18} />
                    )}
                  </div>
                  <div className="flex justify-between items-center text-sm">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-500">
              In 3 simple steps, get your dream home without any brokerage
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Step up to Pune Dream Homes Prime & explore Owner Posted Properties",
                icon: <Star size={32} />,
              },
              {
                id: 2,
                title: "Access Prime Properties",
                desc: "Unlock properties that are exclusively reserved for our Prime members",
                icon: <Unlock size={32} />,
              },
              {
                id: 3,
                title: "Finalize directly",
                desc: "Contact owners directly, visit property and close the deal effortlessly",
                icon: <Users size={32} />,
              },
            ].map((step) => (
              <div
              ...
              >
                <div className="w-16 h-16 bg-blue-50 text-(--color-primary) border-2 border-(--color-primary) rounded-md flex items-center justify-center mb-6 shadow-sm">
                  {step.icon}
                  <span className="absolute -top-3 -right-3 bg-(--color-primary) text-white w-8 h-8 rounded-md flex items-center justify-center font-bold">
                    {step.id}
                  </span>
                </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div
                key={i}
                className="bg-white p-6 rounded-md shadow-md border border-gray-100 flex flex-col justify-between"
              >
                <div>
                  ...
                  <p className="text-gray-600 italic text-xs leading-relaxed mb-6">
                    "{t.text}"
                  </p>
                </div>
        ...
        <div className="max-w-4xl mx-auto bg-(--color-primary-lightest) p-8 rounded-md border border-(--color-primary-light-2)">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1">
              <h3 className="text-2xl font-black text-(--color-primary-dark-2) mb-4 leading-tight">
                Ready to save on brokerage?
              </h3>
              <div className="flex items-center gap-3 bg-white p-3 rounded-md mb-4 border border-blue-200">
                <MessageSquare className="text-green-600" size={24} />
                <p className="font-medium text-xs">
                  I agree to receive updates from Pune Dream Homes via WhatsApp
                </p>
                <input
                  type="checkbox"
                  defaultChecked
            ...
            <div className="w-full md:w-64 bg-white p-6 rounded-md border border-blue-200 shadow-sm">
              <p className="text-xs font-bold text-gray-500 mb-4 text-center">
                Were you assisted by a Tele-executive?
              </p>
              <div className="flex gap-2">
                <button className="flex-1 py-2 border rounded-md hover:bg-slate-50 transition-colors">
                  No
                </button>
                <button className="flex-1 py-2 border border-(--color-primary) text-(--color-primary) font-bold rounded-md hover:bg-blue-50 transition-colors">
                  Yes
                </button>
              </div>
            </div>
        ...
        <div className="mt-12 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="text-(--color-primary) font-bold hover:underline">
                Request a Callback
              </button>
            </div>
          </div>
